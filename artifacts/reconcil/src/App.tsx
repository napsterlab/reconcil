...
import {
  ...
  MoreHorizontal, PackageOpen, Plus, ReceiptText, Search, Send, Settings2, ShieldCheck,
  Sparkles, Target, Trash2, Upload, UserPlus, Users, X, RefreshCw,
} from 'lucide-react';
import {
  ...
  useCreateClient,
  useCreateManualMatch, useDeleteMatch, useDeleteTeamMember, useGetClient, useGetDashboardSummary, useGetMe,
  ...
  useMarkAllNotificationsRead, useMarkNotificationRead, useRegisterCabinet,
  useResendTeamInvitation, useRunReconciliation,
...
function Team() {
  ...
  return <><PageHeading eyebrow="Le cabinet" title="Une équipe alignée." description="Invitez vos collaborateurs et donnez à chacun une vue claire sur les contrôles à terminer." action={<button onClick={() => setOpen(true)} data-testid="button-invite-member" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground shadow-[3px_3px_0_hsl(var(--secondary))]"><UserPlus className="h-4 w-4" /> Inviter un collaborateur</button>} /><QueryState loading={team.isLoading} error={team.isError} empty={!team.isLoading && !team.data?.length} retry={() => team.refetch()}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(team.data as TeamMember[] | undefined)?.map((member) => <div key={member.id} data-testid={`card-team-member-${member.id}`} className="group relative rounded-2xl border border-border bg-card p-5 hover:border-secondary/70"><div className="flex items-start justify-between"><Avatar initials={member.initials} tone={member.role === 'admin' ? 'gold' : 'dark'} /><div className="relative"><button onClick={() => setMenuId((current) => current === member.id ? null : member.id)} aria-label={`Actions pour ${member.name}`} aria-expanded={menuId === member.id} data-testid={`button-member-options-${member.id}`} className={cn('rounded-md p-1 text-muted-foreground hover:bg-muted group-hover:opacity-100', menuId === member.id ? 'bg-muted opacity-100' : 'opacity-0')}><MoreHorizontal className="h-4 w-4" /></button>{menuId === member.id && <div data-testid={`menu-member-actions-${member.id}`} className="absolute right-0 top-8 z-20 w-48 overflow-hidden rounded-xl border border-border bg-card p-1 shadow-xl">{member.status === 'invitation' && <button onClick={() => resend.mutate({ memberId: member.id }, { onSuccess: () => { setMenuId(null); void queryClient.invalidateQueries({ queryKey: getListTeamQueryKey() }); } })} disabled={resend.isPending} data-testid={`button-resend-invite-${member.id}`} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold hover:bg-muted disabled:opacity-50"><Send className="h-3.5 w-3.5 text-accent" /> Renvoyer l’invitation</button>}<button onClick={() => { if (window.confirm(`Retirer ${member.name} de l’équipe ?`)) remove.mutate({ memberId: member.id }, { onSuccess: () => { setMenuId(null); void queryClient.invalidateQueries({ queryKey: getListTeamQueryKey() }); } }); }} disabled={remove.isPending} data-testid={`button-remove-member-${member.id}`} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /> Retirer de l’équipe</button></div>}</div></div><p className="mt-5 text-sm font-bold">{member.name}</p><p className="mt-1 text-xs text-muted-foreground">{member.email}</p><div className="mt-5 flex items-center justify-between border-t border-border pt-3"><span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{member.role === 'admin' ? 'Administrateur' : 'Collaborateur'}</span><span className={cn('flex items-center gap-1.5 text-[10px] font-semibold', member.status === 'actif' ? 'text-accent' : 'text-muted-foreground')}><span className={cn('h-1.5 w-1.5 rounded-full', member.status === 'actif' ? 'bg-accent' : 'bg-muted-foreground')} />{member.status === 'actif' ? 'Actif' : 'Invitation envoyée'}</span></div></div>)}</div></QueryState>{open && <Modal title="Inviter un collaborateur" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><Field label="Nom complet" value={form.name} required placeholder="Ex. Salma El Idrissi" onChange={(v) => setForm({ ...form, name: v })} testId="input-member-name" /><Field label="Adresse email" value={form.email} required type="email" placeholder="salma@cabinet.ma" onChange={(v) => setForm({ ...form, email: v })} testId="input-member-email" /><label className="block"><span className="mb-1.5 block text-xs font-semibold">Rôle</span><select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as 'admin' | 'collaborateur' })} data-testid="select-member-role" className="h-10 w-full rounded-lg border border-border bg-background px-3 py-1.5 text-sm"><option value="collaborateur">Collaborateur</option><option value="admin">Administrateur</option></select></label><button disabled={invite.isPending} data-testid="button-send-invite" className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-bold text-primary-foreground disabled:opacity-60">{invite.isPending && <Loader2 className="h-4 w-4 animate-spin" />}Envoyer l’invitation</button></form></Modal>}</>;
}
...
function Login() {
  ...
  return <div className="grain flex min-h-[100dvh] bg-primary text-primary-foreground"><div className="hidden w-[43%] flex-col justify-between border-r border-primary-foreground/10 p-10 lg:flex"><Logo /><div className="max-w-md"><div className="mb-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.2em] text-secondary"><span className="h-1.5 w-1.5 rounded-full bg-secondary" /> Rapprochement bancaire</div><h1 className="font-serif text-[52px] font-bold leading-[.95] tracking-[-.06em]">Le contrôle<br /><span className="text-secondary">enfin lisible.</span></h1><p className="mt-6 max-w-sm text-sm leading-relaxed text-primary-foreground/60">Reconcil accompagne les cabinets marocains dans leur contrôle mensuel, avec la rigueur d’un grand livre et la simplicité d’une bonne feuille de calcul.</p><div className="mt-10 flex items-center gap-5 text-xs text-primary-foreground/50"><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-secondary" /> Compatible Excel, Sage, EBP</span><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-secondary" /> Données maîtrisées</span></div></div><p className="text-[10px] uppercase tracking-[.16em] text-primary-foreground/30">Reconcil · Casa / Rabat / partout au Maroc</p></div><div className="flex flex-1 items-center justify-center p-6 sm:p-10"><div className="w-full max-w-[390px]"><div className="mb-10 lg:hidden"><Logo /></div><div className="mb-8"><p className="mb-2 text-[10px] font-bold uppercase tracking-[.18em] text-secondary">Espace cabinet</p><h2 className="font-serif text-3xl font-bold tracking-[-.04em]">Bon retour.</h2><p className="mt-2 text-sm text-primary-foreground/55">Connectez-vous pour retrouver vos dossiers.</p></div><form onSubmit={submit} className="space-y-4"><label className="block"><span className="mb-2 block text-xs font-semibold text-primary-foreground/75">Email professionnel</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} data-testid="input-login-email" className="h-12 w-full rounded-xl border border-primary-foreground/15 bg-primary-foreground/10 px-4 text-sm outline-none placeholder:text-primary-foreground/30 focus:border-secondary" /></label><label className="block"><span className="mb-2 block text-xs font-semibold text-primary-foreground/75">Mot de passe</span><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} data-testid="input-login-password" className="h-12 w-full rounded-xl border border-primary-foreground/15 bg-primary-foreground/10 px-4 text-sm outline-none focus:border-secondary" /></label>{login.isError && <p data-testid="status-login-error" className="rounded-lg bg-destructive/20 px-3 py-2 text-xs text-red-100">Email ou mot de passe incorrect.</p>}<button disabled={login.isPending} data-testid="button-login-submit" className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-secondary text-sm font-bold text-primary shadow-[3px_3px_0_hsl(var(--accent))] disabled:opacity-60">{login.isPending && <Loader2 className="h-4 w-4 animate-spin" />}Ouvrir mon espace</button></form><div className="mt-6 rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 p-4 text-center"><p className="text-xs text-primary-foreground/55">Vous n’avez pas encore de cabinet ?</p><Link href="/register" data-testid="link-register-cabinet" className="mt-2 inline-flex text-xs font-bold text-secondary hover:underline">Créer un espace cabinet <ArrowUpRight className="ml-1 h-3.5 w-3.5" /></Link></div><p className="mt-5 text-center text-xs text-primary-foreground/40">Démo : <span className="text-primary-foreground/65">demo@cabinet-atlas.ma</span> · mot de passe <span className="text-primary-foreground/65">demo</span></p></div></div></div>;
}

function Register() {
  const [, setLocation] = useLocation();
  const register = useRegisterCabinet();
  const [form, setForm] = useState({ cabinetName: '', name: '', email: '', password: '', acceptedTerms: false });
  const submit = (e: FormEvent) => { e.preventDefault(); register.mutate({ data: form }, { onSuccess: () => setLocation('/') }); };
  const errorMessage = register.isError ? ((register.error as { data?: { error?: string } })?.data?.error || 'Impossible de créer le cabinet. Vérifiez les informations saisies.') : '';
  return <div className="grain flex min-h-[100dvh] bg-primary text-primary-foreground"><div className="hidden w-[43%] flex-col justify-between border-r border-primary-foreground/10 p-10 lg:flex"><Logo /><div className="max-w-md"><div className="mb-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.2em] text-secondary"><span className="h-1.5 w-1.5 rounded-full bg-secondary" /> Nouveau cabinet</div><h1 className="font-serif text-[52px] font-bold leading-[.95] tracking-[-.06em]">Votre contrôle,<br /><span className="text-secondary">enfin posé.</span></h1><p className="mt-6 max-w-sm text-sm leading-relaxed text-primary-foreground/60">Créez votre espace cabinet en quelques secondes et commencez à structurer vos rapprochements mensuels.</p></div><p className="text-[10px] uppercase tracking-[.16em] text-primary-foreground/30">Reconcil · Casa / Rabat / partout au Maroc</p></div><div className="flex flex-1 items-center justify-center p-6 sm:p-10"><div className="w-full max-w-[430px]"><div className="mb-8 lg:hidden"><Logo /></div><Link href="/login" data-testid="link-back-login" className="mb-8 inline-flex items-center gap-2 text-xs font-semibold text-primary-foreground/55 hover:text-primary-foreground"><ArrowLeft className="h-4 w-4" /> Retour à la connexion</Link><div className="mb-7"><p className="mb-2 text-[10px] font-bold uppercase tracking-[.18em] text-secondary">Création d’espace</p><h2 className="font-serif text-3xl font-bold tracking-[-.04em]">Bienvenue chez Reconcil.</h2><p className="mt-2 text-sm text-primary-foreground/55">Les informations de votre cabinet resteront modifiables.</p></div><form onSubmit={submit} className="space-y-3.5"><Field label="Nom du cabinet" value={form.cabinetName} required placeholder="Ex. Cabinet Atlas Conseil" onChange={(v) => setForm({ ...form, cabinetName: v })} testId="input-register-cabinet-name" /><Field label="Votre nom complet" value={form.name} required placeholder="Ex. Nadia El Mansouri" onChange={(v) => setForm({ ...form, name: v })} testId="input-register-name" /><Field label="Email professionnel" value={form.email} required type="email" placeholder="vous@cabinet.ma" onChange={(v) => setForm({ ...form, email: v })} testId="input-register-email" /><Field label="Mot de passe" value={form.password} required type="password" placeholder="8 caractères minimum" onChange={(v) => setForm({ ...form, password: v })} testId="input-register-password" /><label className="flex items-start gap-2 pt-1 text-[11px] text-primary-foreground/60"><input type="checkbox" checked={form.acceptedTerms} onChange={(e) => setForm({ ...form, acceptedTerms: e.target.checked })} required data-testid="input-register-terms" className="mt-0.5 accent-secondary" /><span>J’accepte les conditions d’utilisation et la politique de confidentialité de Reconcil.</span></label>{errorMessage && <p data-testid="status-register-error" className="rounded-lg bg-destructive/20 px-3 py-2 text-xs text-red-100">{errorMessage}</p>}<button disabled={register.isPending} data-testid="button-register-submit" className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-secondary text-sm font-bold text-primary shadow-[3px_3px_0_hsl(var(--accent))] disabled:opacity-60">{register.isPending && <Loader2 className="h-4 w-4 animate-spin" />}Créer mon espace cabinet</button></form></div></div></div>;
}
